...
def Changepassword():
    ...
    if len(t1.get())==0:
       ...
       t1.focus_set()
        ...
        cur=conn.cursor()
        cur.execute("select * from stdregdform where Userid=%s and Password=%s",(t1.get(),t2.get()))
        results=cur.fetchall()
        if len(results)!=0:
            Userid=t1.get();
            New_Password=t3.get();

            sql="Update stdregdform set Password=%s where Userid=%s";
...
l3=Label(win,text="CHANGE PASSWORD(STUDENT)",fg="black",bg="#B2A59B",font=("calibri",18,"bold"))
...
b2=Button(win,text="CANCEL",command=Cancel,fg="black",bg="light blue",font=("calibri",15,"bold"))
