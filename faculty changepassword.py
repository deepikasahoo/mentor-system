...
def Changepassword():
    ...
    if len(t1.get())==0:
       ...
       t1.focus_set()
        ...
        cur=conn.cursor()
        cur.execute("select * from facultyregform where User_id=%s and Password=%s",(t1.get(),t2.get()))
        results=cur.fetchall()
        if len(results)!=0:
            User_id=t1.get();
            New_Password=t3.get();

            sql="Update facultyregform set Password=%s where User_id=%s";
            ...
            mb.showerror("","Wrong User_id& Password")
...
l3=Label(win,text="CHANGE PASSWORD(FACULTY)",fg="black",bg="#B2A59B",font=("calibri",18,"bold"))
...
b2=Button(win,text="CANCEL",fg="black",bg="light blue",font=("calibri",15,"bold"))
b2.place(x=280,y=410)
b3=Button(win,text="<==",fg="black",bg="red",command=Back,font=("calibri",15,"bold"))
